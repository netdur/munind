use munind::index::{NgtIdenticalObjectEdgeType, NgtIndexType};
use munind::{NgtDistanceType, NgtIndex, NgtProperty, NgtSearchOptions};

#[test]
fn test_basic_insert_search() {
    let mut property = NgtProperty::new(2);
    property.set_distance_type(NgtDistanceType::L2);
    property.edge_size_for_creation = 4;
    property.edge_size_for_search = 4;

    let mut index =
        NgtIndex::create_graph_and_tree("./target/ngt_test", property).expect("create index");
...
fn test_save_open() {
    ...
    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_test_save", property).unwrap();
    index.insert(&[0.0, 0.0]).unwrap();
    index.insert(&[1.0, 1.0]).unwrap();
    index.build_index();
    index.save(Some("./target/ngt_test_save.bin")).unwrap();

    let loaded = NgtIndex::open("./target/ngt_test_save.bin").unwrap();
...
fn test_cosine_normalizes_inserted_objects_and_queries() {
    let mut property = NgtProperty::new(2);
    property.set_distance_type(NgtDistanceType::Cosine);
    property.edge_size_for_creation = 4;
    property.edge_size_for_search = 4;

    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_test_cosine", property).unwrap();
...
fn test_default_property_matches_ngt_defaults() {
    let property = NgtProperty::new(16);
...
#[test]
fn test_save_open_ngt_directory_layout() {
    let mut property = NgtProperty::new(2);
    property.set_distance_type(NgtDistanceType::Cosine);
    property.edge_size_for_creation = 3;
    property.edge_size_for_search = 3;

    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_dir", property).unwrap();
    index.insert(&[1.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
    index.insert(&[1.0, 1.0]).unwrap();
    index.build_index();
    index.save_as_ngt("./target/ngt_dir").unwrap();

    let loaded = NgtIndex::open_ngt("./target/ngt_dir").unwrap();
    assert_eq!(loaded.objects.len(), 3);
    assert!(loaded.tree.is_some());

    let options = munind::NgtSearchOptions {
        k: 1,
        epsilon: 0.0,
        edge_size: Some(3),
    };
...
fn test_identical_object_directed_edge_behavior() {
    let mut property = NgtProperty::new(2);
    property.identical_object_edge_type = NgtIdenticalObjectEdgeType::DirectedEdge;

    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_identical", property).unwrap();
    index.insert(&[1.0, 0.0]).unwrap();
    index.insert(&[1.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
    index.build_index();
...
fn test_tree_splits_and_returns_leaf_seeds() {
    ...
    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_tree_split", property).unwrap();
    index.insert(&[0.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
...
fn test_graph_only_build_does_not_create_tree() {
    let mut property = NgtProperty::new(2);
    property.index_type = NgtIndexType::Graph;
    property.edge_size_for_creation = 3;

    let mut index = NgtIndex::create_graph_and_tree("./target/ngt_graph_only", property).unwrap();
    index.insert(&[0.0, 0.0]).unwrap();
    index.insert(&[1.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
    index.build_index();
...
fn test_tree_guided_insertion_uses_current_object_leaf() {
    ...
    let mut index =
        NgtIndex::create_graph_and_tree("./target/ngt_tree_insertion", property).unwrap();
    index.insert(&[0.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
    index.insert(&[10.0, 10.0]).unwrap();
    index.insert(&[10.0, 11.0]).unwrap();
...
fn test_parallel_batch_build_produces_graph() {
    ...
    let mut index =
        NgtIndex::create_graph_and_tree("./target/ngt_parallel_build", property).unwrap();
    index.insert(&[0.0, 0.0]).unwrap();
    index.insert(&[1.0, 0.0]).unwrap();
    index.insert(&[0.0, 1.0]).unwrap();
    index.insert(&[1.0, 1.0]).unwrap();
